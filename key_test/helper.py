...
def is_exist(driver,tag,content):
    ...
    try:  
        ...
        print 'is_exist %s: %s is not found'%(tag,content)
        
        
        
        
        
def runcase(driver,lines):
    result=False
    tag,value,action,sleep_time,assert_tag,assert_value,assert_attr,n=lines.split(',')
    print 'tag--->'+tag+'  value--->'+value+'  action--->'+action+'  sleep--->'+sleep_time
    
    #操作
    if 'text' in tag:
        if 'click' in action:
            is_exist(driver, 'name', value).click()
            
  
    if "id" in tag:
        if 'click' in action:
            is_exist(driver, 'id', value).click()
        if 'clear' in action:
            is_exist(driver, 'id', value).clear()
        if 'input' in action:
            is_exist(driver, 'id', value).send_keys(action.split(':')[-1])
            
    if 'driver' in tag:
        if 'keyevent' in action:
            driver.keyevent(action.split(':')[-1])
    ...
    if 'contain' in assert_tag:
        if assert_value in driver.page_source:
            result=True
        
    if 'id' in assert_tag:
        if 'text' in assert_attr:
            if is_exist(driver,'id',assert_value).text==assert_attr.split(":")[-1]:
                result=True
    if assert_tag=='':
        result=True
